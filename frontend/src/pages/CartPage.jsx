...
import { Trash2, CheckCircle, MapPin, Gift, Plus, Minus } from "lucide-react";
...
const CartPage = () => {
  ...
  useEffect(() => {
    const rawProduct = location.state;

    if (
      rawProduct &&
      rawProduct.productId &&
      (rawProduct.price || rawProduct.originalPrice)
    ) {
      const newProduct = {
        productId: rawProduct.productId,
        name: rawProduct.name || "Unnamed Product",
        image: rawProduct.image || "https://via.placeholder.com/150",
        price: parseFloat(rawProduct.price) || 0,
        originalPrice: parseFloat(rawProduct.originalPrice) || null,
        discount: parseFloat(rawProduct.discount) || 0,
        size: rawProduct.size || "Default",
    ...
    <div className="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
      ...
      {cartItems.length === 0 ? (
        <div className="bg-white p-6 rounded-lg shadow-md text-center">
          <img
            src="https://m.media-amazon.com/images/G/31/VNA/2023/Jupiter/PC/Empty_State_Bag.png"
            alt="Empty Cart"
              ...
              >
                <div className="flex flex-col md:flex-row items-start md:items-center gap-4 w-full">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-600 text-sm">Size: {item.size || "Default"}</p>
                    <p className="text-gray-800 font-bold text-md mb-2">
                      ₹{((item.price ?? item.originalPrice ?? 0) * item.quantity).toFixed(2)}
                    </p>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white p-6 rounded-lg shadow-md sticky top-4">
              ...
              <button
                ...
                disabled={cartItems.length === 0 || !deliveryEstimate}
